...
import type { Instructor, InstructorCreate } from '@/lib/types';
...
export default function InstructorsPage() {
  ...
  const [editingRecord, setEditingRecord] = useState<Instructor | null>(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    try { const res = await instructorApi.list(); setData(Array.isArray(res) ? res : []); }
  ...
  const openEdit = (record: Instructor) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingRecord) {
        ...
        await instructorApi.update(editingRecord.id, rest);
        message.success('更新成功');
      } else {
        await instructorApi.create(values);
  ...
  const handleDelete = async (id: number) => {
    try { await instructorApi.remove(id); message.success('已删除'); fetchData(); }
    catch (e: any) { message.error(e.message || '删除失败'); }
  };

  const filtered = search
    ? data.filter(d => (d.name || '').includes(search) || (d.instructor_code || '').includes(search))
    : data;
  ...
  const columns = [
    ...
    { title: '教练编号', dataIndex: 'instructor_code', width: 120, render: (v: string) => v || '-' },
    { title: '地区', key: 'region', width: 140,
      render: (_: any, r: Instructor) => r.province && r.city ? `${r.province} ${r.city}` : r.province || '-' },
    { title: '单位', dataIndex: 'company_name', width: 150, render: (v: string) => v || '-' },
    { title: '状态', dataIndex: 'is_active', width: 80, align: 'center' as const,
      render: (v: boolean) => <Tag color={v ? 'green' : 'default'}>{v ? '在职' : '停用'}</Tag> },
    { title: '创建时间', dataIndex: 'created_at', width: 170,
      render: (v: string) => v ? v.substring(0, 19).replace('T', ' ') : '-' },
    { title: '操作', key: 'action', width: 150, render: (_: any, r: Instructor) => (
      <Space size={0}>
  ...
  return (
    ...
    <Card
      extra={
        <Space>
          <Input placeholder="搜索用户名/编号" prefix={<SearchOutlined />} style={{ width: 180 }}
            value={search} onChange={e => setSearch(e.target.value)} allowClear />
          <Button icon={<ReloadOutlined />} onClick={fetchData}>刷新</Button>
          <Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>添加教练</Button>
        </Space>
      }
    >
      <Table columns={columns} dataSource={filtered} rowKey="id" loading={loading}
        pagination={{ pageSize: 20, showSizeChanger: true, showTotal: t => `共 ${t} 条` }}
        size="middle" locale={{ emptyText: '暂无教练数据' }} />

      <Modal title={editingRecord ? '编辑教练' : '添加教练'} open={modalOpen}
        ...
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item name="instructor_code" label="教练编号">
          ...
          <Form.Item name="is_active" label="状态">
            <Select options={[{ value: true, label: '在职' }, { value: false, label: '停用' }]} style={{ width: 100 }} />
          </Form.Item>
        </Form>
      </Modal>
    </Card>
